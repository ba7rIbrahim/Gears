  ...
  ({ className, gradient = true, blur = true, ...props }, ref) => {
    return (
      <section
        ref={ref}
        className={cn(
          "relative z-0 flex min-h-[70vh] pt-20 w-full flex-col items-center justify-center overflow-hidden rounded-md bg-background",
          className
        )}
        {...props}
      >
        ...
        >
          <div className="flex flex-col items-center text-center space-y-4">
            <h1
              className={cn(
                "text-5xl md:text-6xl sm:text-5xl lg:text-7xl font-bold tracking-tight"
              )}
            >
              إبحث عن الأجزاء المناسبة لسيارتك.
            </h1>
            <p
              className={cn(
                "text-lg md:text-xl max-w-[600px] text-muted-foreground mt-4"
              )}
            >
              استكشف كتالوجنا الشامل لقطع غيار السيارات من أفضل العلامات
              التجارية. استخدم بحثنا المدعم بالذكاء الاصطناعي للعثور على ما
              تحتاجه بسرعة.
            </p>
            <div className={cn("flex gap-4 mt-8")}>
              <Button variant="outline">
                <Link to="/">جرب الآن</Link>
              </Button>
              <Button variant="default">
                <Link to="/shop">تصفح القطع</Link>
              </Button>
            </div>
          </div>
        </motion.div>
